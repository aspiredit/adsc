...
- [x] **D. Chat widget UI** (AFK) — `js/chat-widget.js`: additive floating bubble, `textContent`-only rendering (XSS-safe), scoped `.adsc-chat-` styles, guarded auto-init, all states (idle/loading/success/refused/429/error). Component built + tested against a mock; **page-wiring + brand-color match happen at integration (after C)**. *Test: `tests/chat-widget.test.js` — 18 tests green, jsdom.*
