...
- [~] **A2. Embed** (HITL) — `scripts/build_embeddings.py` embeds chunks via Workers AI (`@cf/baai/bge-base-en-v1.5`) → `_data/chat_embeddings.json`. Creds read from `.env` (gitignored; template `.env.sample`) or env/CI secrets — never committed. **Code + offline test done; producing the real manifest awaits the Workers AI token.** *Test: `tests/chat-embeddings.test.js` — 4 tests green via deterministic `ADSC_EMBED_FAKE` mode (no token needed). Blocked by: A1.*
