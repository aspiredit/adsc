...
- [x] **A2. Embed** (HITL) — `scripts/build_embeddings.py` embeds chunks via Workers AI (`@cf/baai/bge-base-en-v1.5`) → `_data/chat_embeddings.json` (**178 chunks, dim 768, ~2.8 MB**). Creds read from `.env` (gitignored; template `.env.sample`) — never committed. End-to-end spot-check confirmed correct top-hit retrieval for real questions. *Test: `tests/chat-embeddings.test.js` — 4 tests green via deterministic `ADSC_EMBED_FAKE` mode (no token needed).*
...
- [x] **C. Serverless RAG endpoint** (HITL) — `worker/index.js` + `worker/wrangler.jsonc`, **deployed** at `https://adsc-chat.krand03.workers.dev/ask`. Embeds question (bge) → `retrieve.js` top-k over the bundled manifest → grounds `@cf/meta/llama-3.2-3b-instruct` (3.1-8b was deprecated 2026-05-30). Guardrail system prompt + Worker-controlled medical/legal deflection (lenient `REFUSE` detection), per-IP rate limit (15/min), CORS locked to ADSC origins, free tier = hard spend cap. Live-verified: grounded answers cite correctly, medical/legal refuse consistently. *Test: `tests/chat-worker.test.js` — 11 tests, mocked AI binding.*
  - *Slice-E refinement noted:* "I don't have that" / deflection answers still attach low-relevance citations — suppress those in the safety pass.
- [x] **D. Chat widget UI** (AFK) — `js/chat-widget.js`: additive floating bubble, `textContent`-only rendering (XSS-safe), scoped `.adsc-chat-` styles, guarded auto-init, all states (idle/loading/success/refused/429/error). *Test: `tests/chat-widget.test.js` — 18 tests green, jsdom.*
- [x] **D↔C integration** — widget brand-matched (`#1F5E96` Carolina blue-deep + gold focus) and embedded on `index.html` via the existing ES-module `init` block, pointed at the live `workers.dev` endpoint. Verified end-to-end in jsdom against the **live** Worker: grounded answers render with citation links; medical question renders as a styled refusal notice with no citations.
- [x] **E. Safety & cost guardrail pass** (HITL) — hardened + live-verified:
  - **Medical/legal refusal** — reordered the system prompt to *safety-first* (refusal outranks the not-in-context rule) **and** added a deterministic high-precision keyword backstop (`MEDICAL_LEGAL_RE`) that refuses medication/dosage/legal questions *before* any AI call. Live battery (melatonin, meds, ABA-therapy choice, lawsuit) all refuse with the professional-referral disclaimer.
  - **Citation correctness** — added a `NO_INFO` marker: off-corpus / "I don't know" answers return a controlled message with **no citations** (fixes the low-relevance-citation leak).
  - **Cost cap** — Cloudflare's rate-limit binding is *approximate by design* (per-colo, burst-permissive — [docs](https://developers.cloudflare.com/workers/runtime-apis/bindings/rate-limit/)), so it dampens sustained abuse but won't block short bursts. **The hard spend cap is the free tier: with no payment method on the account, Workers AI cannot bill — it just stops at the daily allocation.** Rate limiter kept as a best-effort dampener (15/min per IP).
  - **Privacy/disclaimer** — no separate privacy page exists, so a one-line disclaimer lives in the widget panel: AI-generated + not medical/legal advice + don't share personal/health details.
  - *Tests:* worker 16 + widget 19; full suite **269 green**. **Ready to merge to `main`** (go-live). *Blocked by: D.*
...
- [ ] **G. Automated KB refresh** (HITL setup, then AFK) — GitHub Action mirroring `build-manifests.yml`: on push to `main` touching `_data/**` (events/blogs) or `index.html` (FAQ), re-run `build_chat_index.py` + `build_embeddings.py`, commit the refreshed manifests, and `wrangler deploy` the Worker so the chatbot's knowledge stays current (esp. **events**, which change often). Needs a Cloudflare token (Workers AI + Workers Scripts) as a GitHub Actions secret. **Runs on `main`, so it lands with/after the merge (deploys the Worker that's currently branch-only).** *Blocked by: E (merge).*
